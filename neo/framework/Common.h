...
public:
	 ...
	 *  of gbDoom3 doesn't support the given CallBackType and the Mod must either error out
	 *  or handle the case that the callback doesn't work)
	 *
	 * Of course this means that for every new SourceHook hack a Mod (that's ported to gbDoom3)
	 * uses, a corresponding entry must be added to enum CallbackType and it must be handled,
	 * which implies that the Mod will only properly work with the latest gbDoom3 git code
	 * or the next release..
	 ...
	 * between gbDoom3 and the Game DLLs; the alternative would be something like
	 ...
	 * from gbDoom3 that Mods can call (and that's not exported via the normal interface classes).
