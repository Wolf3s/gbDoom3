...
int Sys_GetSystemRam( void ) {
    /* There is no longer any 'nice' way to get
     this kind of system info on a Mac
     without using gestalt.  There is IOKit
     but it is just a nicer hack than a usable API
     So I will leave this empty for Macs..  who cares really
     even Macs from 2013 have 4GB so it isn't
...
NSString *getCPUType(void) {
    ...
    {
            [cpu appendString:@"APPLE SILICON"];
    }
    return [cpu autorelease];
}

bool OSX_GetCPUIdentification( void ) {
    getCPUType();
	return true;
}
