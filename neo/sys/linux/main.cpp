...
bool Sys_GetPath(sysPath_t type, idStr &path) {
	...
	case PATH_CONFIG:
		...
		if (s)
			idStr::snPrintf(buf, sizeof(buf), "%s/Doom 3", s);
		else
			idStr::snPrintf(buf, sizeof(buf), "%s/.config/Doom 3", getenv("HOME"));
	...
	case PATH_SAVE:
		...
		if (s)
			...
			idStr::snPrintf(buf, sizeof(buf), "%s/.local/share/Doom 3", getenv("HOME"));
