...
#error "adapt homedir code for wchar_t!"
	...
	// DG: use "My Documents/My Games/Doom 3" to write stdout.txt and stderr.txt
	//     instead of the binary, which might not be writable
	Sys_GetHomeDir(path, sizeof(path));

	if (_stat(path, &st) == -1) {
		// oops, "My Documents/My Games/Doom 3" doesn't exist - does My Games/ at least exist?
		...
		_mkdir(path); // create My Documents/My Games/Doom 3/
	}
